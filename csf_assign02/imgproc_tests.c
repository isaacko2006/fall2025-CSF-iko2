...
void test_get_attributes(TestObjs *objs)
{
  ...
  (void) objs;
  // pixel layout: R G B A (each 8 bits)
  uint32_t pixel = 0x11223344;
  // r = 11, g = 22, b = 33, a = 44

  ASSERT(get_r(pixel) == 0x11);
  ASSERT(get_g(pixel) == 0x22);
  ASSERT(get_b(pixel) == 0x33);
  ASSERT(get_a(pixel) == 0x44);

  // min and max values to test
  uint32_t pixel2 = 0xFF00FF00;
  ASSERT(get_r(pixel2) == 0xFF);
  ASSERT(get_g(pixel2) == 0x00);
...
void test_emboss_gray(TestObjs *objs)
{
  // silence objs parameter because it had to be passed in for declaration, but isn't needed in helper
  (void)objs;
  // if neighbor pixel equals current pixel then diff = 0 and gray = 128
  uint32_t p1 = 0x808080FF;
  ASSERT(emboss_gray(p1, p1) == 128);

  // positive diff in red
  uint32_t curr = 0x000000FF;  // black
  uint32_t neigh = 0xFF0000FF; // red
  // diffr much greater than 255, must clamp at 255
  ASSERT(emboss_gray(curr, neigh) == 255);

  // negative diff in blue
  curr = 0x0000FFFF;  // blue
  neigh = 0x000000FF; // black
  // diffb very negative, must clamp to 0
  ASSERT(emboss_gray(curr, neigh) == 0);

  // red and green diffs equal in value (tie)
  curr = 0x000000FF;  // black
  neigh = 0x101000FF; // r=16, g=16, b=0
